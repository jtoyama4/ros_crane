...
class RL(object):
    ...
    def build_actor_network(self):
        ...
        final_model.add(Dense(10, activation='tanh', input_dim=self.sensor_dim))
        final_model.add(Dense(10, activation='relu'))
        
        final_model.add(Dense(self.action_dim, activation='tanh'))
        
        s_i_a = tf.placeholder(tf.float32, [None, self.action_dim])
        
        actions = final_model([s_i_a])

        actions = tf.mul(actions, 3.0)

        return final_model, s_i_a, actions

    def build_critic_network(self):
        sensor_model = Sequential()
        sensor_model.add(Dense(10, activation='tanh', input_dim=self.sensor_dim))

        action_model = Sequential()
        action_model.add(Dense(10, activation='tanh', input_dim=self.action_dim))
        ...
        s_i_c = tf.placeholder(tf.float32, [None, self.sensor_dim])
    ...
    def get_action(self, image_state):
        if self.t % self.freq == 0:
            ...
            action += np.random.normal(scale=0.7, size=self.action_dim)
        else:
    ...
    def build_training_op(self):
        ...
        gvs = optimizer.compute_gradients(critic_loss, var_list=self.critic_network_weights)
        capped_gvs = [(tf.clip_by_value(gra, -0.01, 0.01), var) for gra, var in gvs]
        ...
        capped_grads =[(tf.clip_by_value(gra, -0.001, 0.001), var) for gra, var in grads]
        actor_optimize = tf.train.AdamOptimizer(self.actor_lr).apply_gradients(capped_grads)
    ...
    def run(self):
        ...
        if self.t > self.initial_replay_size:
            ...
            self.init_arm()
        termination = False
        next_image_state = self.get_sensor_state()
        while not termination:
            image_state = self.get_sensor_state()
            ...
            reward = self.get_reward(next_image_state)
            r_mean += reward
            print "Time:%d Action:%s Reward:%f Angle:%s" % (self.t, action, reward, next_image_state)
